...
const createCallableEndpoint =
  (accessToken: string, endpoint: Endpoints) =>
  (params: unknown) => (endpoints[endpoint] as Function)(accessToken, params)

export default function MonzoClient(accessToken: string): IMonzoClient {
  const callableEndpoints = Object.keys(endpoints).map(
    (name) => [name, createCallableEndpoint(accessToken, name as Endpoints)]
  )
